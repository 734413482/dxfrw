...
//            qDebug()<<"detected on disk change";
		...
		if (!actualName.isEmpty())
		{
			...
			qDebug() << "RS_Graphic::save: Can't create object! File not saved!";
		}
		...
		qDebug() << "RS_Graphic::save: Done!";
	}
	else {
		RS_DEBUG->print("RS_Graphic::save: File not modified, not saved");
		qDebug() << "RS_Graphic::save: File not modified, not saved";
		ret = true;
	}
...
bool RS_Graphic::saveAs(const QString &filename, RS2::FormatType type, bool force)
{
	...
	qDebug() << "RS_Graphic::saveAs: Entering...";
